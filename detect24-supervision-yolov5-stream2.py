# versi stream cctv via virtual cam obs
...
save_interval = 300                     # Metadata baru, disimpan tiap interval
...
upload_buffer = deque(maxlen=6) # Buffer untuk mengisi metadata sebelum diupload
...
for path, im, im0s, vid_cap, _ in dataset:
    ...
    if pred is not None and len(pred):  # Logika untuk cek apakah ada hasil prediksi yang valid
        ...
        # === Update ID unik
        for c, tid in zip(tracked.class_id, tracked.tracker_id):    # Loop semua hasil tracking: ambil class dan tracker ID-nya
            if c not in unique_objects:     # Jika class (misal 'Orang') belum ada di dictionary
                unique_objects[c] = set()   # Inisialisasi set baru untuk class tersebut
            unique_objects[c].add(tid)      # Tambahkan tracker ID ke set → menghindari duplikat

        labels = [f" #ID:{tid} {names[c]} [{conf:.2f}]" # Buat label per objek: tampilkan ID, nama class, dan confidence
        ...
        cv2.putText(im0s, fps_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 0), 2)
    ...
    if current_time - last_save_time >= save_interval:  # Cek apakah waktu interval penyimpanan metadata sudah tercapai
        ...
        # Reset unique_objects untuk interval berikutnya
        unique_objects = {}
